...
function PmsExampleAttachFile({ boardFiles, mode, fnChangeFile, fnDeleteFile, posblAtchFileNumber, title, id }) {
            ...
            <dd>
                <span className="file_attach">
                    {(mode === CODE.MODE_CREATE) &&
						<>
                        {/* <label className="btn btn_dark_h46 w_100" htmlFor="egovComFileUploader1">파일추가</label> */}
						<input name="file_0" id={id} type="file" multiple onChange={e => onChangeFileInput(e)}></input>
						총 업로드 가능한 첨부파일 개수는 {posblAtchFileNumber} 개 입니다.<br></br>
						</>
					}
                    {/* 첨부파일 1개 당  filesTag는 3개 요소(span, button, br)를 가진다 */}
                    {(mode === CODE.MODE_MODIFY && (filesTag.length/3 < posblAtchFileNumber)) &&
						<> 
                        {/* <label className="btn btn_dark_h46 w_100" htmlFor="egovComFileUploader2">파일추가</label> */}
						<input name="file_1" id={id} type="file" multiple onChange={e => onChangeFileInput(e)}></input>
...
export default React.memo(PmsExampleAttachFile);